import random
import cv2
import cvzone
from cvzone.HandTrackingModule import HandDetector
import time

cap = cv2.VideoCapture(0)
cap.set(3, 640)
cap.set(4, 480)

detector = HandDetector(maxHands=1)

timer = 0
stateResult = False
startGame = False
scores = [0, 0]  # [AI, Player]
randomNumber = 0  # Initialize randomNumber
imgAI = None  # Initialize imgAI

while True:
    imgBG = cv2.imread("BG.png")
    if imgBG is None:
        print("Error: Could not load BG.png. Check the file path.")
        break
    success, img = cap.read()
    
    if not success:
        print("Failed to read from camera")
        break

    imgScaled = cv2.resize(img, (0, 0), None, 0.875, 0.875)
    imgScaled = imgScaled[:, 80:480]

    # Find Hands
    hands, img = detector.findHands(imgScaled)  # with draw

    if startGame:
        if not stateResult:
            timer = time.time() - initialTime
            cv2.putText(imgBG, str(int(timer)), (605, 435), cv2.FONT_HERSHEY_PLAIN, 6, (255, 0, 255), 4)

            if timer > 3:
                stateResult = True
                timer = 0

                if hands:
                    playerMove = None
                    hand = hands[0]
                    fingers = detector.fingersUp(hand)
                    if fingers == [0, 0, 0, 0, 0]:
                        playerMove = 1  # Rock
                    elif fingers == [1, 1, 1, 1, 1]:
                        playerMove = 2  # Paper
                    elif fingers == [0, 1, 1, 0, 0]:
                        playerMove = 3  # Scissors
                    elif fingers == [0, 1, 0, 0, 0]:
                        playerMove = 3  # Scissors (alternative gesture)
                    elif fingers == [1, 1, 0, 0, 0]:
                        playerMove = 3  # Scissors (another alternative)

                    if playerMove is not None:
                        randomNumber = random.randint(1, 3)
                        print(f"AI Move: {randomNumber}")  # Debug print
                        imgAI = cv2.imread(f'Resources/{randomNumber}.png', cv2.IMREAD_UNCHANGED)
                        if imgAI is None:
                            print(f"Error: Could not load Resources/{randomNumber}.png")
                        
                        # Player Wins
                        if (playerMove == 1 and randomNumber == 3) or \
                           (playerMove == 2 and randomNumber == 1) or \
                           (playerMove == 3 and randomNumber == 2):
                            scores[1] += 1

                        # AI Wins
                        elif (playerMove == 3 and randomNumber == 1) or \
                             (playerMove == 1 and randomNumber == 2) or \
                             (playerMove == 2 and randomNumber == 3):
                            scores[0] += 1
                        # Note: Draw doesn't change scores

    imgBG[234:654, 795:1195] = imgScaled

    # FIXED: This condition was the main issue
    if stateResult and imgAI is not None:
        try:
            # Add debug print to see what's happening
            print(f"Displaying AI image: Resources/{randomNumber}.png")
            imgBG = cvzone.overlayPNG(imgBG, imgAI, (149, 310))
        except Exception as e:
            print(f"Error overlaying PNG: {e}")

    cv2.putText(imgBG, str(scores[0]), (410, 215), cv2.FONT_HERSHEY_PLAIN, 4, (255, 255, 255), 6)
    cv2.putText(imgBG, str(scores[1]), (1112, 215), cv2.FONT_HERSHEY_PLAIN, 4, (255, 255, 255), 6)

    cv2.imshow("BG", imgBG)

    key = cv2.waitKey(1)
    if key == ord('s'):
        startGame = True
        initialTime = time.time()
        stateResult = False
        imgAI = None  # Reset AI image for new game
        print("Game started! Show your hand gesture in 3 seconds...")
    elif key == ord('q') or key == 27:  # 'q' or ESC key to exit
        break
    elif key == ord('r'):  # Reset game
        scores = [0, 0]
        startGame = False
        stateResult = False
        imgAI = None
        print("Game reset!")

cap.release()
cv2.destroyAllWindows()